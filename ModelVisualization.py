...
# Add model name here
modelName = ""  # Add complete name of pytorch model on which we need to perform the visualization

# Global Variables
CLEAN_MODEL_FILEPATH = "/home/bt3/19CS10049/backdoorPytorch/Models/{}.pt".format(modelName)
DATA_DIR = '/home/bt3/19CS10049/backdoorPytorch/GTSRB_data/'  # data folder
ORIGINAL_TRIGGER = '/home/bt3/19CS10049/backdoorPytorch/Triggers/OriginalTrigger.png'
RV_TRIGGER = '/home/bt3/19CS10049/backdoorPytorch/Triggers/RVTrigger{}.png'.format(modelName)
...
spec = SourceFileLoader("CompactTransformer","./Models/CompactTransformer.py").load_module()
model = spec.CCT().float().to(device)

def getActivationMaps(model, test_gen):
    model.eval()
    test_gen.on_epoch()
    X, Y = test_gen.gen_data()
    X = X.to(device)
    Y = Y.to(device)
    with torch.no_grad():
        _ = model(X)
        activationMaps = model.getActivationMaps()
