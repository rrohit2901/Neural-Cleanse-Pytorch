...
def make_mask_pattern(image_row, image_col, channel_num, margin, pattern_size):   # This function creates a white square on bottom right pattern
    mask = Image.open("/RTrigger/cnnMask.png")
    mask = mask.resize((IMG_SHAPE[0], IMG_SHAPE[1]))
    mask = np.array(mask).astype('float32')

    Pattern = Image.open("/RTrigger/cnnPattern.png")
    Pattern = Pattern.resize((IMG_SHAPE[0], IMG_SHAPE[1]))
    Pattern = np.array(Pattern).astype('float32')
